...
export const useChat = ({ 
  ...
  const fetchQuickActions = useCallback(async () => {
    try {
      const response = await makeAuthenticatedRequest(`${apiBaseUrl}/chatbot/quick-actions`)
  ...
  }, [apiBaseUrl, makeAuthenticatedRequest, setQuickActions, setConnected, onConnectionChange, onError])
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      const response = await makeAuthenticatedRequest(`${apiBaseUrl}/chatbot/chat`, {
        method: 'POST',
        body: JSON.stringify({
          message: content,
      ...
      addMessage('❌ Error de conexión. Verifica que el servidor esté funcionando.', 'bot', 'error')
      setConnected(false)
      onConnectionChange?.(false)
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      const response = await makeAuthenticatedRequest(`${apiBaseUrl}/chatbot/analytics`)
